...
class DepthCalculator:
    ...
    def process_stereo_image(self, image_path: str, 
        ...
        # 注意：split_image已经会将图保存到debug_output_dir（如果提供）
        # 下面的代码是为了确保在output目录下也有副本（使用更清晰的命名）
        
        # 清理：如果创建了临时旋转文件，可以选择删除（可选）
        # 这里保留旋转后的文件，以便调试和查看
        if self.enable_debug:
            print(f"原始图像尺寸: {orig_width}x{orig_height}")
            print(f"分割图目录: {split_output_dir}")
        
        # 1.5. 在debug模式下，将split后的图保存到output目录
        if self.enable_debug:
            import shutil
            # 优先使用debug_output_dir，否则使用split_output_dir的父目录
            if debug_output_dir:
                save_output_dir = Path(debug_output_dir)
            else:
                save_output_dir = Path(split_output_dir).parent
            save_output_dir.mkdir(parents=True, exist_ok=True)
            
            quadrant_names = ["左上", "右上", "左下", "右下"]
            for i, (quadrant_path, name) in enumerate(zip(quadrants, quadrant_names), start=1):
                src_path = Path(quadrant_path)
                if src_path.exists():
                    # 复制到output目录，使用更清晰的命名
                    base_name = os.path.splitext(os.path.basename(image_path))[0]
                    dst_filename = f"{base_name}_split_{i}_{name}.jpg"
                    dst_path = save_output_dir / dst_filename
                    shutil.copy2(str(src_path), str(dst_path))
                    print(f"💾 已保存split后的图: {dst_filename} ({name})")
