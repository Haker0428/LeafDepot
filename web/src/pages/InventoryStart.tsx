...
export default function InventoryStart() {
  ...
  useEffect(() => {
    const fetchTaskNo = async () => {
      try {
        const res = await fetch(`${GATEWAY_URL}/inventory/next-task-no`);
        if (res.ok) {
          const data = await res.json();
          setTaskNoInput(data.taskNo);
        }
      } catch {
        // 失败时降级使用本地生成
        setTaskNoInput(generateTaskNo());
      }
    };
    fetchTaskNo();
  }, []);
  ...
  // 生成任务清单
  const createTaskMainfest = (taskNo: string) => {
    // 检查盘点任务表格是否为空
    ...
    try {
      ...
      localStorage.setItem("currentTaskNo", taskNo);
  ...
  const handleStartInventory = async () => {
    ...
    // 1. 先从服务端获取正确的任务号（保证全局递增同步）
    let actualTaskNo = taskNoInput;
    try {
      const res = await fetch(`${GATEWAY_URL}/api/inventory/next-task-no`);
      if (res.ok) {
        const data = await res.json();
        if (data.taskNo) {
          actualTaskNo = data.taskNo;
          setTaskNoInput(actualTaskNo);
        }
      }
    } catch {}

    // 2. 生成任务清单（使用正确的任务号）
    const manifest = createTaskMainfest(actualTaskNo);

    if (manifest) {
      // 保存任务编号到本地存储，作为全局变量
      localStorage.setItem("currentTaskNo", actualTaskNo);
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (res.ok) {
                            const data = await res.json();
                            setTaskNoInput(data.taskNo);
                            toast.success(`已生成新任务号: ${data.taskNo}`);
                          }
                        } catch {
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {binsData.map((item, index) => (
                            <tr
                              ...
                              onClick={() => handleBinSelect(item.id)}
                            >
                              <td
