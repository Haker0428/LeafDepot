...
export default function UserManage() {
    ...
    try {
      ...
      if (response.ok && result.code === 200) {
        ...
        await addOperationLog({
          ...
          action: "添加用户",
          target: newUserForm.userName,
          status: "success",
          details: {
            new_user_code: newUserForm.userCode,
            new_user_name: newUserForm.userName,
            new_user_level: newUserForm.userLevel,
            new_user_mobile: newUserForm.mobile,
  ...
  const handleDeleteUsers = async () => {
    ...
    // 获取要删除的用户名列表（用于日志记录）
    const usersToDelete = users.filter(u => selectedUsers.includes(u.userCode));
    const userNamesToDelete = usersToDelete.map(u => u.userName).join(', ');
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select
                      value={newUserForm.deptName}
                      onChange={(e) =>
                        handleFormChange("deptName", e.target.value)
                      }
                      className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-300 focus:ring-green-500 focus:border-green-500 transition-all appearance-none bg-white"
                    >
                      <option value="衡水市局">衡水市局</option>
                      <option value="河北省局">河北省局</option>
                    </select>
