...
typedef struct tagFrameInfo {
  ...
  ULONG Magic;
  USHORT FrameType;  /* I frames , P frames or BBP frames Audio frames or dsp
                        status etc */
  ULONG Length;      /*lenth include this header */
  ULONG FrameNumber; /* serial number of this frame */
  UCHAR Breakable;   /* indicate if stream breakable, you could restart new
                        file(with PktSysHeader) if true */
  /*ULONG   Ack;*/
...
#define _OSD_YEAR4 \
  (_OSD_BASE + 0) /*show year time by length of 4 , for example: 2005*/
#define _OSD_YEAR2 \
  (_OSD_BASE + 1) /*show year time by length of 2, for example: 05    */
#define _OSD_MONTH3 \
  (_OSD_BASE + 2) /*show month time in English, for example: Jan*/
#define _OSD_MONTH2 \
  (_OSD_BASE + 3) /*show month time by two Arabic numerals, for example: 07*/
#define _OSD_DAY \
  (_OSD_BASE + 4) /*show day time by two Arabic numerals, for example: 31*/
...
#define _OSD_MINUTE (_OSD_BASE + 9)  /*show minute time by length of 2: \
                                        00��59*/
#define _OSD_SECOND (_OSD_BASE + 10) /*show second time by length of 2: \
                                        00��59*/
#define _OSD_MILISECOND \
  (_OSD_BASE + 11) /*show millisecond time by length of 3: 000~999*/
#define _OSD_APM \
  (_OSD_BASE + 14) /*show a.m. or p.m. by length of 2 bit, AM or PM*/

// For new added APIs to set OSD: SetEncoderOsdDisplayMode,
// SetDecoderOsdDisplayMode and SetDisplayOsdDisplayMode in v6.0 SDK, we use new
// basic address of special character.
